...
class TvSeriesFavoriteViewModelTest {

    private lateinit var viewModel: TvSeriesFavoriteViewModel
    ...
    @Before
    fun setup(){
        viewModel = TvSeriesFavoriteViewModel(filmRepository)
    }

    @Test
    fun testGetFavorites() {
        val expected = MutableLiveData<PagedList<Film>>()
        expected.value = PagedTestDataSources.snapshot(DataDummy.generateDummyTvSeries())

        `when`(filmRepository.getFavoriteTvSeries()).thenReturn(expected)
